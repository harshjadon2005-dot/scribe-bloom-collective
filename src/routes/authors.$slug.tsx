  ...
  notFoundComponent: () => (
    <SiteLayout>
      <Container className="py-32 text-center">
        ...
        <Link to="/authors" className="mt-6 inline-block border-b border-accent pb-1">
          All authors
        </Link>
      </Container>
    </SiteLayout>
...
function AuthorPage() {
  ...
  const popularArticles = articles
    .filter((a) => a.trending || a.editorsPick || a.featured)
    .slice(0, 3);
  ...
  const relatedTopics = categories
    .filter((c) =>
      author.expertise.some(
        (e) =>
          e.toLowerCase() === c.name.toLowerCase() ||
  ...
  const relatedAuthors = authors
    .filter((a) => a.slug !== author.slug && a.expertise.some((e) => author.expertise.includes(e)))
    .slice(0, 3);
    ...
    <SiteLayout>
      <Container size="wide" className="pt-16 pb-24 border-b border-border">
        <nav
          ...
          className="text-xs uppercase tracking-widest text-ink-soft mb-12"
        >
        ...
        <div className="grid md:grid-cols-12 gap-16 items-start">
          ...
          <div className="md:col-span-7 flex flex-col justify-center h-full pt-8">
            ...
            <p className="text-xl text-ink-soft leading-relaxed max-w-2xl mb-10">{author.bio}</p>

            <div className="mb-12">
              <div className="text-[10px] uppercase tracking-widest text-ink-soft mb-4">
                Areas of Expertise
              </div>
              <div className="flex flex-wrap gap-2">
                {author.expertise.map((e: string) => (
                  <span
                    key={e}
                    className="text-xs uppercase tracking-widest px-4 py-2 border border-border rounded-full hover:border-accent hover:text-accent transition-colors cursor-default bg-background"
                  >
                    {e}
                  </span>
                ))}
      ...
      <Container size="wide" className="py-24">
        <div className="grid lg:grid-cols-12 gap-16">
            ...
            <div className="mb-12 border-b border-border pb-6 flex justify-between items-end">
              <div>
                ...
                <p className="text-ink-soft">
                  The most recent dispatches from {author.name.split(" ")[0]}.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-8 mb-24">
              {latestArticles.map((article) => (
                ...
                >
                  ...
                  <p className="text-ink-soft line-clamp-2 max-w-3xl mb-4">{article.excerpt}</p>
          ...
          <div className="lg:col-span-4 space-y-16 pt-6">
            {/* Timeline */}
            <div>
              <div className="text-xs font-semibold uppercase tracking-widest border-b border-border pb-4 mb-6">
                Career Highlights
              </div>
              <div className="space-y-6 relative before:absolute before:inset-0 before:ml-2 before:-translate-x-px md:before:mx-auto md:before:translate-x-0 before:h-full before:w-0.5 before:bg-gradient-to-b before:from-transparent before:via-border before:to-transparent">
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group is-active">
                  ...
                  <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] ml-4 md:ml-0 bg-secondary/30 p-4 border border-border">
                    ...
                    <div className="font-medium text-foreground mt-1">
                      Lead coverage on Agentic AI Frameworks
                    </div>
                  </div>
                </div>
                <div className="relative flex items-center justify-between md:justify-normal md:odd:flex-row-reverse group">
                  ...
                  <div className="w-[calc(100%-2rem)] md:w-[calc(50%-1.5rem)] ml-4 md:ml-0 bg-secondary/30 p-4 border border-border">
                    ...
                    <div className="font-medium text-foreground mt-1">
                      Joined Lordiphosa Editorial Board
                    </div>
                  </div>
                </div>
              </div>
