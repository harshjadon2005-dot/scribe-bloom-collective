      ...
      <Container size="wide" className="pb-24">
        <div className="grid lg:grid-cols-12 gap-14 rule-t pt-16">
          <aside className="lg:col-span-4 space-y-8">
            {[
              { label: "Letters", email: "letters@lordiphosa.com", note: "Reader mail. The best ones appear in the Sunday dispatch." },
              { label: "Pitches", email: "pitches@lordiphosa.com", note: "Two paragraphs, not a finished essay. Reply within two weeks." },
              { label: "Corrections", email: "corrections@lordiphosa.com", note: "We fix and note them at the foot of each piece." },
              { label: "Press", email: "press@lordiphosa.com", note: "For interviews and inquiries." },
            ].map((c) => (
              <div key={c.label} className="rule-b pb-6">
                <div className="eyebrow" style={{ color: "var(--ink-soft)" }}>{c.label}</div>
                <a href={`mailto:${c.email}`} className="mt-1 block font-display text-xl hover:text-accent transition-colors break-all">
                  {c.email}
                </a>
          ...
          >
            ...
            <div>
              <label className="eyebrow block mb-2" style={{ color: "var(--ink-soft)" }}>Your letter</label>
              <textarea
                rows={8}
...
function Field({ label, type = "text", placeholder }: { label: string; type?: string; placeholder?: string }) {
  return (
    <div>
      <label className="eyebrow block mb-2" style={{ color: "var(--ink-soft)" }}>{label}</label>
      <input
        type={type}
