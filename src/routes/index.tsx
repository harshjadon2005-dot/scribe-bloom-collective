...
export const Route = createFileRoute("/")({
  loader: async () => {
    ...
    const trending = await getArticles({ data: { trending: true, limit: 4, excludeSlug: featured.slug } });
    const latest = await getArticles({ data: { sort: 'newest', limit: 4, excludeSlug: featured.slug } });
    const editorsPicks = await getArticles({ data: { editorsPick: true, limit: 3, excludeSlug: featured.slug } });
...
function Index() {
  const { featured, trending, latest, editorsPicks, continueReading, stats, allArticles, authors, categories } = Route.useLoaderData();
  ...
  return (
    <SiteLayout>
      ...
      <div className="rule-b bg-secondary/40 hover-pause relative border-b border-border">
        <Container size="wide" className="px-0 md:px-6">
          <div className="flex items-center text-[11px] uppercase tracking-[0.18em] text-ink-soft h-11 overflow-hidden">
            ...
            {/* Left Fixed Section */}
            <div className="shrink-0 flex items-center h-full px-4 md:px-0 md:pr-6 bg-secondary/40 relative z-10 
      ...
      <Container size="wide" className="pb-16 animate-in fade-in slide-in-from-bottom-4 duration-700 delay-150 fill-mode-both">
        <div className="max-w-4xl mx-auto text-center mb-12">
          ...
          <p className="mt-6 text-lg sm:text-xl text-ink-soft leading-relaxed max-w-2xl mx-auto">
            Independent publication delivering trusted insights on artificial intelligence, engineering, business innovation, software development, cybersecurity, and the technologies shaping the future.
          </p>
          <div className="mt-8 flex flex-wrap items-center justify-center gap-4">
            <Link to="/articles" className="inline-flex items-center justify-center h-11 px-8 rounded-sm bg-foreground text-background font-medium text-sm transition-transform hover:-translate-y-0.5">
              Explore Latest Articles
            </Link>
            <Link to="/topics" className="inline-flex items-center justify-center h-11 px-8 rounded-sm border border-border bg-background text-foreground font-medium text-sm transition-all hover:bg-secondary hover:-translate-y-0.5">
              Browse Topics
            </Link>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 rule-t pt-8">
          ...
          <div className="bg-secondary/30 p-6 rounded-sm border border-border transition-all duration-300 hover:-translate-y-1 hover:shadow-sm">
            ...
            <ul className="space-y-3">
              {["AI Agents", "Quantum Computing", "Cybersecurity", "Semiconductors", "Startups", "Cloud Computing"].map((topic, i) => (
                <li key={i}>
                  <Link to="/search" className="text-sm text-ink-soft hover:text-foreground relative group inline-flex items-center gap-2">
          ...
          <div className="bg-secondary/30 p-6 rounded-sm border border-border transition-all duration-300 hover:-translate-y-1 hover:shadow-sm">
            ...
            <div className="flex flex-wrap gap-2">
              {["Artificial Intelligence", "Software Engineering", "Programming", "Business", "Data Science", "Future Trends", "Cloud", "Robotics"].map((topic, i) => (
                <Link key={i} to="/topics" className="px-3 py-1.5 rounded-full border border-border/80 bg-background text-[11px] font-medium text-ink-soft hover:border-accent hover:text-accent transition-colors">
                  {topic}
                </Link>
              ))}
            </div>
          ...
          <div className="bg-secondary/30 p-6 rounded-sm border border-border transition-all duration-300 hover:-translate-y-1 hover:shadow-sm flex flex-col justify-between">
            <div>
              ...
              <p className="text-sm text-ink-soft leading-relaxed italic">
                "Every day we publish carefully curated insights covering emerging technologies, engineering, innovation, and digital transformation."
              </p>
            </div>
            <Link to="/about" className="mt-6 text-xs font-medium text-foreground inline-flex items-center gap-1 group">
              Read More
      ...
      <div className="bg-secondary/40 rule-t rule-b">
        <Container size="wide" className="py-24">
          <div className="mb-12">
            <div className="eyebrow mb-3 uppercase tracking-[0.18em] text-[10px] font-semibold text-accent">Verified Writers</div>
            ...
            <p className="mt-4 text-ink-soft max-w-2xl leading-relaxed text-lg">
              The journalists, researchers, engineers and industry experts shaping Lordiphosa's reporting.
            </p>
          </div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-8 items-start relative z-10">
                      {a.portrait ? (
                        <div className="w-32 h-32 sm:w-40 sm:h-40 shrink-0 overflow-hidden rounded-sm border border-border">
                          <img src={a.portrait} alt={a.name} className="w-full h-full object-cover grayscale opacity-90 group-hover:grayscale-0 group-hover:opacity-100 transition-all duration-500" />
                        </div>
                      ) : (
                      ...
                      <div>
                        <div className="flex items-center gap-3 flex-wrap mb-2">
                          <h3 className="font-display text-3xl group-hover:text-accent transition-colors">{a.name}</h3>
                          {a.stats?.badges?.[0] && (
                        ...
                        <div className="text-xs uppercase tracking-[0.15em] text-ink-soft font-semibold">{a.role}</div>
                        
                        <div className="mt-4 flex flex-wrap gap-2">
                          {a.expertise.map((exp) => (
                            <span key={exp} className="text-[10px] font-medium uppercase tracking-wider px-2 py-1 border border-border/80 bg-secondary/50 rounded-sm text-ink-soft group-hover:border-accent/40 group-hover:text-foreground transition-colors">
                              {exp}
                            </span>
                          ))}
                        </div>
                        ...
                        <div className="mt-8 grid grid-cols-2 gap-4 rule-t pt-4">
                          <div>
                            <div className="text-2xl font-mono text-foreground">{a.stats?.articles || 0}</div>
                            <div className="text-[10px] uppercase tracking-widest text-ink-soft">Articles</div>
                          </div>
                          <div>
                            <div className="text-2xl font-mono text-foreground">{a.stats?.reads || "0"}</div>
                            <div className="text-[10px] uppercase tracking-widest text-ink-soft">Total Reads</div>
                          </div>
                        </div>

                        {latestArticle && (
                          <div className="mt-8 rule-t pt-4">
                            <div className="text-[10px] uppercase tracking-widest text-ink-soft mb-2">Latest Article</div>
                    ...
                    <div className="flex flex-col sm:flex-row gap-6 items-start sm:items-center">
                      ...
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center justify-between gap-4">
                          <div>
                            <h3 className="font-display text-2xl group-hover:text-accent transition-colors truncate">{a.name}</h3>
                            <div className="text-[10px] uppercase tracking-[0.15em] text-ink-soft font-semibold truncate mt-1">
                              {a.role}
                            </div>
                          </div>
                          <div className="hidden sm:block text-right">
                            <div className="text-lg font-mono text-foreground">{a.stats?.articles || 0}</div>
                            <div className="text-[9px] uppercase tracking-widest text-ink-soft">Articles</div>
                          </div>
                        </div>

                        <div className="mt-3 flex flex-wrap gap-2">
                          {a.expertise.map((exp) => (
                            <span key={exp} className="text-[10px] font-medium uppercase tracking-wider text-ink-soft group-hover:text-foreground transition-colors">
                              {exp}
                            </span>
                          ))}
                        </div>
                        ...
                        {latestArticle && (
                          <div className="mt-4 pt-3 border-t border-border/50 text-sm">
                            <span className="text-[10px] uppercase tracking-widest text-ink-soft mr-2">Latest:</span>
      ...
      <Container size="wide" className="pb-24">
        <div className="mb-12 flex flex-col md:flex-row md:items-end justify-between gap-6">
          <div>
            <div className="eyebrow mb-3 uppercase tracking-[0.18em] text-[10px] font-semibold text-accent">Popular Topics</div>
          ...
          <Link to="/topics" className="text-sm font-medium hover:text-accent transition-colors whitespace-nowrap group inline-flex items-center gap-1">
            View all topics <span className="group-hover:translate-x-1 transition-transform">→</span>
          </Link>
        </div>
                  ...
                  <div className="p-6 lg:p-8 flex-1 flex flex-col justify-between">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-6">
                        <div>
                          <span className="text-lg font-mono text-foreground mr-1">{topic.articleCount}</span>
                          <span className="text-[9px] uppercase tracking-widest text-ink-soft">Articles</span>
                        </div>
                      ...
                      {latestArticle && (
                        <div className="mb-6 pt-5 border-t border-border/50">
                          <div className="text-[9px] uppercase tracking-widest text-ink-soft mb-1.5">Latest</div>
                      ...
                      <div className="flex items-center text-[10px] font-medium uppercase tracking-widest text-accent">
                        Explore Topic <span className="ml-1.5 group-hover:translate-x-1.5 transition-transform">→</span>
                      </div>
                    </div>
                  </div>
                </div>
