      ...
      scripts: [
        {
          ...
          children: JSON.stringify({
            ...
            author: { "@type": "Person", name: loaderData?.authors?.find(a => a.slug === a.authorSlug)?.name },
...
function ArticlePage() {
  ...
  const related = allArticles.filter(a => a.category === article.category && a.slug !== article.slug).slice(0, 4);
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 lg:grid-cols-10 gap-10 lg:gap-14">
          ...
          <aside className="md:col-span-1 lg:col-span-2">
            <div className="sticky top-28 space-y-12">
              ...
              <div className="hidden md:block">
                <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-4">On This Page</div>
              ...
              <div className="hidden md:block">
                ...
                <div className="mt-8 flex flex-wrap gap-3">
                  <button onClick={() => window.alert('Article saved to bookmarks!')} aria-label="Save" title="Save" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-ink-soft hover:border-accent hover:text-accent hover:bg-background transition-colors bg-secondary/30">
                    <Bookmark className="w-4 h-4" />
                  </button>
                  <button onClick={() => { navigator.clipboard.writeText(window.location.href); window.alert('Link copied to clipboard!'); }} aria-label="Copy Link" title="Copy Link" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-ink-soft hover:border-accent hover:text-accent hover:bg-background transition-colors bg-secondary/30">
                    <LinkIcon className="w-4 h-4" />
                  </button>
                  <button onClick={() => window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(article.title)}&url=${encodeURIComponent(window.location.href)}`, '_blank')} aria-label="Share" title="Share" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-ink-soft hover:border-accent hover:text-accent hover:bg-background transition-colors bg-secondary/30">
                    <Twitter className="w-4 h-4" />
                  </button>
                  <button onClick={() => window.print()} aria-label="Print" title="Print" className="w-9 h-9 rounded-full border border-border flex items-center justify-center text-ink-soft hover:border-accent hover:text-accent hover:bg-background transition-colors bg-secondary/30">
              ...
              <div className="hidden md:block space-y-4 text-xs">
                <div className="flex justify-between border-b border-border/50 pb-2">
                  ...
                  <span className="font-medium text-foreground">{formatDate(article.publishedAt)}</span>
                </div>
                <div className="flex justify-between border-b border-border/50 pb-2">
                  <span className="text-ink-soft">Updated</span>
                  <span className="font-medium text-foreground">{article.updatedAt ? formatDate(article.updatedAt) : formatDate(article.publishedAt)}</span>
                </div>
                <div className="flex justify-between border-b border-border/50 pb-2">
          ...
          <main className="md:col-span-3 lg:col-span-6 min-w-0">
            <article className="max-w-[720px] mx-auto">
              ...
              {/* Header */}
              <nav aria-label="Breadcrumb" className="text-[10px] uppercase tracking-widest text-ink-soft mb-6 flex flex-wrap items-center gap-2">
              ...
              {category && (
                <div className="mb-4">
                  <Link to="/categories/$slug" params={{ slug: category.slug }} className="text-accent text-[11px] font-semibold uppercase tracking-[0.2em] hover:underline">
                    {category.name}
                  </Link>
                </div>
              )}
              ...
              <div className="md:hidden mb-10 border border-border rounded-sm bg-secondary/10">
                <details className="group">
                  <summary className="font-medium p-4 cursor-pointer list-none flex items-center justify-between text-sm">
                    <span className="text-[10px] uppercase tracking-widest font-semibold text-accent">On This Page</span>
                    <span className="transition group-open:rotate-180">↓</span>
                  </summary>
                  <ul className="px-4 pb-4 space-y-3 text-sm border-t border-border pt-4">
                    {toc.map((t) => (
                      <li key={t.id} className={t.level === 3 ? "pl-4" : ""}>
                        <a href={`#${t.id}`} className="text-ink-soft hover:text-foreground block">{t.text}</a>
                      </li>
                    ))}
                  </ul>
                </details>
              ...
              <div className="prose-editorial max-w-none">
                ...
                {article.body.split("\n\n").slice(1).map((block: string, i: number) => {
                  ...
                  if (trimmed.startsWith("## ")) {
                    ...
                    return <h2 key={i} id={id} className="scroll-mt-32">{text}</h2>;
                  }
                  if (trimmed.startsWith("### ")) {
                    const text = trimmed.slice(4);
                    const id = text.toLowerCase().replace(/[^a-z0-9]+/g, "-");
                    return <h3 key={i} id={id} className="scroll-mt-32">{text}</h3>;
                  }
                  if (trimmed.startsWith("> ")) {
              ...
              <div className="mt-16 rule-t pt-8 grid grid-cols-2 gap-6">
                {prev && (
                  <Link to="/articles/$slug" params={{ slug: prev.slug }} className="group">
                    <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-2">← Previous</div>
                ...
                {next && (
                  <Link to="/articles/$slug" params={{ slug: next.slug }} className="group text-right col-start-2">
                    <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-2">Next →</div>
                    <div className="font-display sm:text-xl leading-tight group-hover:text-accent transition-colors">
                      {next.title}
                    </div>
                  </Link>
          ...
          <aside className="hidden lg:block lg:col-span-2">
            <div className="sticky top-28 space-y-12">
              ...
              {/* Related Articles */}
              <div>
                <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-5">Related Reads</div>
                <div className="space-y-5">
                  {related.slice(0, 4).map(a => (
                    <Link key={a.slug} to="/articles/$slug" params={{ slug: a.slug }} className="group flex gap-4 items-center">
                      <div className="w-14 h-14 shrink-0 rounded-sm overflow-hidden bg-secondary border border-border">
                        <img src={a.cover} alt="" loading="lazy" className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" />
                      </div>
                      <div className="flex-1 min-w-0">
                        <h4 className="font-display leading-tight group-hover:text-accent transition-colors line-clamp-2 text-sm">{a.title}</h4>
                        <div className="text-[10px] font-mono text-ink-soft mt-1.5 uppercase tracking-wider">{a.readingMinutes} min read</div>
                      </div>
                    </Link>
                  ))}
                </div>
              ...
              {/* Featured Author */}
              <div>
                <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-4">The Author</div>
                <div className="p-5 border border-border bg-secondary/20 rounded-sm">
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <div className="text-[10px] uppercase tracking-widest text-ink-soft mt-0.5">{author.role}</div>
                    </div>
                  </div>
                  <p className="text-xs text-ink-soft leading-relaxed mb-4 line-clamp-3">
                    {author.bio}
                  </p>
                  <Link to="/authors/$slug" params={{ slug: author.slug }} className="text-xs font-semibold text-accent hover:underline">
                    View All Articles →
                  </Link>
                </div>
              </div>
              ...
              {/* Popular Topics */}
              <div>
                <div className="text-[10px] uppercase tracking-widest font-semibold text-accent mb-4">Popular Topics</div>
                <div className="flex flex-wrap gap-2">
                  {popularTopics.slice(0, 7).map(t => (
                    <Link key={t.slug} to="/categories/$slug" params={{ slug: t.slug }} className="text-[11px] font-medium px-3 py-1.5 border border-border rounded-full hover:border-accent hover:text-accent transition-colors bg-background">
                      {t.name}
                    </Link>
                  ))}
                </div>
        ...
        <div className="grid sm:grid-cols-2 gap-10 rule-t pt-12">
            ...
            <div className="space-y-5">
              {related.slice(0, 4).map(a => (
                <Link key={a.slug} to="/articles/$slug" params={{ slug: a.slug }} className="group flex gap-4 items-center">
                  ...
                  <div className="flex-1 min-w-0">
                    <h4 className="font-display leading-tight group-hover:text-accent transition-colors line-clamp-2">{a.title}</h4>
                    <div className="text-[10px] font-mono text-ink-soft mt-1.5 uppercase tracking-wider">{a.readingMinutes} min read</div>
                  </div>
                </Link>
              ))}
          ...
          <div className="space-y-10">
            ...
            <div>
              ...
              <div className="p-6 border border-border bg-secondary/20 rounded-sm">
                <div className="flex items-center gap-3 mb-4">
                  ...
                  <div>
                    <div className="font-display font-medium text-lg leading-tight">{author.name}</div>
                    <div className="text-[10px] uppercase tracking-widest text-ink-soft mt-0.5">{author.role}</div>
                  </div>
                </div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2">
                {popularTopics.map(t => (
                  <Link key={t.slug} to="/categories/$slug" params={{ slug: t.slug }} className="text-xs font-medium px-3 py-1.5 border border-border rounded-full hover:border-accent hover:text-accent transition-colors bg-background">
                    {t.name}
                  </Link>
                ))}
              </div>
